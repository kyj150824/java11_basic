	...
	static class Inner {
		...
		void inMethod() {
			...
			// System.out.println(" 내부에서 외부 클래스의 인스턴스 변수 : num = " + num);
				// -> 공유하는 데이터가 아니기 때문에 불가능. 인스턴스가 아니기 때문에 불가능.
...
public class StaticInnerClassExam {
	...
	public static void main(String[] args) {
		...
		Outer2.outMethod();
		// Outer2.Inner.inMethod(); - inMethod() 는 인스턴스 메소드이므로 정적 클래스인 Inner
		// out.Inner.inMethod(); - Inner 가  정적 클래스이므로 out 인스턴스를 생성하지 않고, Outer2
		
		Outer2.Inner inner = new Outer2.Inner();
		// 내부 클래스의 인스턴스
		inner.inMethod();
		// 외부 클래스 에서 외부 메소드는 모두 실행 가능하며, 내부 메소드 및 멤버 변수의 접근 및 실행이 불가능.
		// 내부 클래스로 생성된 인스턴스는 내부 메소드 및 멤버 변수의 접근 및 실행이 가능하다.
	}
